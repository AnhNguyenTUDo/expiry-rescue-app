<template>
  <div class="relative group block h-full">
    <div class="absolute top-2 left-2 -right-1 -bottom-1 rounded-lg bg-gradient-to-r from-green-500 to-cyan-500 blur-sm opacity-45 group-hover:opacity-60 transition duration-1000 group-hover:duration-400"></div>
    <NuxtLink
    ...
    >
      ...
      {{ formatHour(supermarket.operatingHoursFrom) }} - {{ formatHour(supermarket.operatingHoursTo) }}
    </div>
  </NuxtLink>
  </div>
</template>
